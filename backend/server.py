...
try:
    from database_realtime import firebase_db  # Using Firebase Realtime Database
except Exception as e:
    print(f"Firebase Realtime Database not available: {e}. Using mock database.")
    from mock_realtime_db import firebase_db  # Fallback to mock database
from models import (
...
# Load JWT configuration from environment (NO DEFAULTS for security)
JWT_SECRET_KEY = os.getenv("JWT_SECRET_KEY")
if not JWT_SECRET_KEY or len(JWT_SECRET_KEY) < 32:
    raise ValueError("JWT_SECRET_KEY must be set in environment and at least 32 characters")
...
# Admin credentials for email/password login (bcrypt hashed)
ADMIN_EMAIL = os.getenv("ADMIN_EMAIL")
ADMIN_PASSWORD_HASH = os.getenv("ADMIN_PASSWORD_HASH")
if not ADMIN_EMAIL or not ADMIN_PASSWORD_HASH:
    raise ValueError("Admin credentials (ADMIN_EMAIL and ADMIN_PASSWORD_HASH) must be set in environment")
...
def get_client_ip(request):
    ...
    # Check for forwarded IP headers first (common in Kubernetes/proxy setups)
...
async def ensure_json_response(request, call_next):
    """Ensure all API responses are JSON formatted and include security headers"""
    try:
...
@limiter.limit("5/minute")  # Rate limiting: 5 login attempts per minute
async def admin_login(request: Request, login_data: AdminLoginRequest):
    """Admin login endpoint with bcrypt password verification and rate limiting"""
    try:
        ...
        # Create access token with reduced expiration time
...
async def create_appointment(appointment_data: AppointmentCreate):
    ...
    try:
        ...
        # Get all appointments from Realtime Database
        all_appointments = firebase_db.get('appointments')
        if all_appointments:
            for app_id, app_data in all_appointments.items():
                if (app_data.get('appointment_datetime_utc') == utc_datetime.isoformat() and 
                    app_data.get('status') in ['pending', 'confirmed']):
                    raise HTTPException(
        ...
        firebase_db.set(f'appointments/{appointment.id}', appointment.dict())
    ...
    try:
        ...
        if all_appointments:
            for app_id, app_data in all_appointments.items():
                ...
                if status_filter and app_data.get('status') != status_filter:
                    continue
                appointments.append(Appointment(**app_data))
...
async def update_appointment_status(appointment_id: str, status_update: dict, current_user: str = Depends(verify_token)):
    ...
    try:
        ...
        appointment_data = firebase_db.get(f'appointments/{appointment_id}')
        ...
        # Update status
        firebase_db.update(f'appointments/{appointment_id}', {'status': new_status})
...
async def create_testimonial(testimonial_data: TestimonialCreate, current_user: str = Depends(verify_token)):
    ...
    try:
        ...
        # Save to Firebase Realtime Database
        firebase_db.set(f'testimonials/{testimonial.id}', testimonial.dict())
...
async def get_testimonials(limit: int = 100):
    ...
    try:
        # Get all testimonials from Realtime Database
        all_testimonials = firebase_db.get('testimonials')
        
        testimonials = []
        if all_testimonials:
            for test_id, test_data in all_testimonials.items():
                testimonials.append(Testimonial(**test_data))
...
async def update_testimonial(testimonial_id: str, testimonial_data: TestimonialCreate, current_user: str = Depends(verify_token)):
    ...
    try:
        ...
        existing_data = firebase_db.get(f'testimonials/{testimonial_id}')
        ...
        # Update with new data, preserving id and created_at
        ...
        firebase_db.update(f'testimonials/{testimonial_id}', updated_testimonial.dict())
...
async def delete_testimonial(testimonial_id: str, current_user: str = Depends(verify_token)):
    ...
    try:
        ...
        firebase_db.delete(f'testimonials/{testimonial_id}')
...
async def create_worked_with_company(company_data: WorkedWithCompanyCreate, current_user: str = Depends(verify_token)):
    ...
    try:
        ...
        # Save to Firebase Realtime Database
        firebase_db.set(f'worked_with_companies/{company.id}', company.dict())
...
async def get_worked_with_companies(limit: int = 100):
    ...
    try:
        # Get all worked with companies from Realtime Database
        all_companies = firebase_db.get('worked_with_companies')
        
        companies = []
        if all_companies:
            for comp_id, comp_data in all_companies.items():
                companies.append(WorkedWithCompany(**comp_data))
        
        # Sort by display_order first, then by created_at
        companies.sort(key=lambda x: (x.display_order or 999, x.created_at), reverse=False)
...
async def update_worked_with_company(company_id: str, company_data: WorkedWithCompanyCreate, current_user: str = Depends(verify_token)):
    ...
    try:
        ...
        existing_data = firebase_db.get(f'worked_with_companies/{company_id}')
        ...
        firebase_db.update(f'worked_with_companies/{company_id}', updated_company.dict())
...
async def delete_worked_with_company(company_id: str, current_user: str = Depends(verify_token)):
    ...
    try:
        ...
        firebase_db.delete(f'worked_with_companies/{company_id}')
