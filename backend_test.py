...
BACKEND_URL = os.environ.get('REACT_APP_BACKEND_URL', 'https://homepage-hero-edit.preview.emergentagent.com')
...
class BackendTester:
    ...
    def test_root_endpoint(self):
        """Test GET /api/ endpoint (root API health check)"""
        try:
            ...
            if response.status_code == 200:
                ...
                if all(field in data for field in required_fields):
                    if (data['success'] == True and 
                        'running successfully' in data['message'].lower() and
                        'version' in data['data'] and
                        'timestamp' in data['data']):
                        self.log_test("Root API Health Check", True, f"Backend is running successfully - {data['message']}")
                    else:
                        self.log_test("Root API Health Check", False, f"Unexpected response structure: {data}", data)
                else:
                    missing_fields = [field for field in required_fields if field not in data]
                    self.log_test("Root API Health Check", False, f"Missing required fields: {missing_fields}", data)
            else:
                self.log_test("Root API Health Check", False, f"HTTP {response.status_code}: {response.text}", response.text)
                
        except requests.exceptions.RequestException as e:
            self.log_test("Root API Health Check", False, f"Request failed: {str(e)}")
    
    def test_status_endpoint_as_fallback(self):
        """Test GET /api/status endpoint as fallback health check"""
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, list):
                    self.log_test("Status Endpoint Fallback", True, f"Status endpoint accessible - returned {len(data)} items")
                else:
                    self.log_test("Status Endpoint Fallback", False, f"Unexpected response type: {type(data)}", data)
            else:
                self.log_test("Status Endpoint Fallback", False, f"HTTP {response.status_code}: {response.text}", response.text)
                
        except requests.exceptions.RequestException as e:
            self.log_test("Status Endpoint Fallback", False, f"Request failed: {str(e)}")
    ...
    def test_cors_headers(self):
        ...
        try:
            ...
            response = requests.options(
                f"{API_BASE_URL}/status",
                headers={
                    'Origin': 'https://homepage-hero-edit.preview.emergentagent.com',
    ...
    def test_admin_login_valid_credentials(self):
        """Test POST /api/admin/login with correct credentials (updated for new credentials)"""
        try:
            login_data = {
    ...
    def test_deprecated_module_fix(self):
        """Test that backend is running properly after deprecated module fix"""
        try:
            # Test that the backend server is responding (indicates deprecated module is working)
            response = requests.get(f"{API_BASE_URL}/", timeout=10)
            
            if response.status_code == 200:
                data = response.json()
                if data.get('success') == True and 'running successfully' in data.get('message', '').lower():
                    self.log_test("Deprecated Module Fix", True, "Backend server is running properly - deprecated module issue resolved")
                else:
                    self.log_test("Deprecated Module Fix", False, f"Backend responding but with unexpected message: {data}")
            else:
                self.log_test("Deprecated Module Fix", False, f"Backend not responding properly: HTTP {response.status_code}")
                
        except requests.exceptions.RequestException as e:
            self.log_test("Deprecated Module Fix", False, f"Backend server not accessible - deprecated module issue may persist: {str(e)}")

    def run_all_tests(self):
        """Run all backend tests"""
        print(f"🚀 Starting Backend API Tests - Deprecated Module Fix Verification")
        print(f"📍 Testing API Base URL: {API_BASE_URL}")
        print("=" * 60)
        
        # Test deprecated module fix first
        self.test_deprecated_module_fix()
        
        # Test basic connectivity
        self.test_root_endpoint()
        self.test_status_endpoint_as_fallback()
